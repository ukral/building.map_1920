...
#####################################################################################
# set directory
dir = "C:/Users/u.kral/ownCloud/03_TU Wien/Zenodo/"
######################################################################################


BSM_1920 <- st_read("C:/Users/u.kral/ownCloud/03_TU Wien/Zenodo/Datasets/Building stock and age 1920/BSM_1920.shp", quiet = T)




# Import dataset
dataset_final <- read.csv(paste(dir, "Datasets/Building stock and age 1920/BSM_1920_attribute_table_final.csv", sep = ""), sep = ";")

# Import SHP file "City boundary 2020"
CB_2020 <- st_read(paste(dir, "Background data/City boundary 2020/CB_2020.shp", sep = ""), quiet = T)

# Import SHP file "Areas out of scope 1920"
AOOS_1920 <- st_read(paste(dir, "Background data/Areas out of scope 1920/AOOS_1920.shp", sep = ""), quiet = T)



# Import SHP file "City boundary 1920"
CB_1920 <- st_read(paste(dir, "Datasets/City boundary 1920/CB_1920.shp", sep = ""), quiet = T)

# Import SHP file "Urban district boundaries 1920" 
UDB_1920 <- st_read(paste(dir, "Datasets/Urban district boundaries 1920/UDB_1920.shp", sep = ""), quiet = T)

SABAM_1920 <- st_read(paste(dir, "Datasets/Scope of analog building age map 1920/SABAM_1920.shp", sep = ""), quiet = T)

DD_T_10 <- read_xlsx(paste(dir, "Background data/Tables.xlsx", sep = ""), sheet = "DD_T_10", col_names =  TRUE, range = "A3:C33")

CB_CB_1920_AT <- read_xlsx(paste(dir, "Background data/Tables.xlsx", sep = ""), sheet = "CB_CB_1920_AT", col_names =  TRUE, range = "A3:B5")

CB_UDB_1920_AT <- read_xlsx(paste(dir, "Background data/Tables.xlsx", sep = ""), sheet = "CB_UDB_1920_AT", col_names =  TRUE, range = "A3:B6")

CB_SABAM_1920_AT <- read_xlsx(paste(dir, "Background data/Tables.xlsx", sep = ""), sheet = "CB_SABAM_1920_AT", col_names =  TRUE, range = "A3:B5")


```

# Introduction {-}
This codebook refers to the data repository [Map of Viennese building stock and its age in the 1920s](https://www.doi.org/10.5281/zenodo.3715200), which includes the following geospatial datasets.

1. City boundary 1920 (CB_1920)
2. Urban district boundaries 1920 (UDB_1920)
3. Scope of analog building age map 1920 (SABAM_1920)
4. Building stock and age 1920 (BSM_1920) and its attribute table (BSM_1920_attribute_table_final)

This codebook specifies the data format, the attribute tables and comments on their fields. This codebook is for users who want to reuse and interpret the data records.
...
## Attribute table
The dataset includes `r ncol(st_drop_geometry(CB_1920))` data fields.

```{r, echo=FALSE}
kable(head(CB_CB_1920_AT, nrow(CB_CB_1920_AT)))
...
## Attribute table
The dataset includes `r ncol(st_drop_geometry(UDB_1920))` data fields.

```{r, echo=FALSE}
kable(head(CB_UDB_1920_AT, nrow(CB_UDB_1920_AT)))
```
...
## Attribute table
The dataset includes `r ncol(st_drop_geometry(SABAM_1920))` data fields.

```{r, echo=FALSE}
kable(head(CB_SABAM_1920_AT, nrow(CB_SABAM_1920_AT)))
...
##	Attribute table
The dataset includes `r nrow(DD_T_10)` data fields.

```{r, echo=FALSE}
kable(head(DD_T_10[,1:2], nrow(DD_T_10)))
...
knitr::include_graphics(here::here(paste(dir, "/Figures/Codebook_figure_FF.png", sep = "")))
```
...
### Address_identical
This data field includes only addresses that can be found in "Address.2019" and "Address_revised". Practically speaking, this addresses were present in 1920 and 2019.

The field covers `r length(unique(dataset_final$Address_identical))` unique records.

### Address_rejected
This data field includes only addresses that can be found in "Address.2019" but not in "Address_revised". Practically speaking, this addresses were present in 2019, but not in 1920.

The field covers `r length(unique(dataset_final$Address_rejected))` unique records.

### Address_added
This data field includes only addresses that can be found in "Address_revised" but not in "Address.2019". Practically speaking, this addresses present in 1920, but not in 2019.

The field covers `r length(unique(dataset_final$Address_added))` unique records.
