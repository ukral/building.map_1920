---
title: "Usage code for data descriptor"
...
# Preface {-}
This R Markdown file is part of the data repository [Map of Viennese building stock and its age in the 1920s (Dataset and Codebook)](https://doi.org/10.5281/zenodo.3715200). This R Markdown is a usage code related to a Data Descriptor, which was submitted to Nature Scientific Data. The heading structure of this R Markdown file corresponds with the Data Descriptor.

To execute the R-Code requires the following:

1. Download and save the data repository from Zenodo on the local computer. The data respository includes all input datasets, which are needed to execute the R-Code.
2. Set the local directory name in [this code chunk](#data_input).

The R Markdown file can be fed to [bookdown](https://bookdown.org/) to generate a readable HTML file.
...
# This chunk imports external datasets.

rm(list=ls()) 

outDir<-"C:/Users/u.kral/ownCloud/03_TU Wien/MUBS/temp/Input data comparative analysis"

#####################################################################################
# specify local directory that includes the files from the Zenodo repository https://www.doi.org/10.5281/zenodo.3715200 

dir_zenodo = "C:/Users/u.kral/ownCloud/03_TU Wien/Zenodo/"
######################################################################################
...
dir_github = "C:/Users/u.kral/ownCloud/03_TU Wien/Github/building.map_1920/"
######################################################################################

#####################################################################################
# specify local directory that covers the output files

dir_output = "C:/Users/u.kral/ownCloud/03_TU Wien/building.map_1920/output"
######################################################################################
...
# wien1920_raw <- read.csv(file=paste(outDir, "/W1920_GES.csv", sep = ""), sep = ";", dec = ".", header = TRUE, stringsAsFactors = FALSE)
# wien1920_raw$area <- gsub(",", ".", wien1920_raw$area)
# wien1920_raw$area <- as.numeric(wien1920_raw$area)
# wien1920_raw$gesid <- gsub("B", "UD", wien1920_raw$gesid)
# wien1920_raw$szbg_bj <- gsub(" ", "", wien1920_raw$szbg_bj)


#wien1920_col_select <- c("gesid", "szbg_id", "UD.1920", "bez", "area", "bp_ma08", "szbg_bj",  "plan_fyr", "editor", "quelle", "planalter",  "Location.city_limits_1920", "Location.abam_1920", "boku1912", "stadtew", "ma37oid","ma37adr", "ma37mov", "Address.2020_corr", "adr_eq", "adr_rj", "adr_ad", "adr_fin", "plan_adr", "Polygon.source.Scale")

wien1920_col_name <- c("ID", "ID.bs", "UD.1920", "UD.2020", "Area", "CD.abam", "CD.bs",  "TP_pub.date_year", "Editor", "Polygon.source_id", "TP_pub.date", "Location.city_limit_1920", "Location.abam_1920", "Location.lsm", "Location.usm", "ID.Address.2019", "Address.2019", "Address.2019_reloc", "Address_reviewed", "Address_identical", "Address_rejected", "Address_added", "Address.1920", "Address_revised", "Polygon.source.Scale")

# 
# wien1920_col <- data.frame(wien1920_col_select, wien1920_col_name)
# colnames(wien1920_col) <- c("col_names_raw_data", "col_names_release")
# 
# wien1920 <- wien1920_raw[, wien1920_col_select]
# colnames(wien1920) <- wien1920_col_name
# 
# #### Modifikationen für wien1920
# pos_temp <- which(wien1920$TP_pub.date_year == 2018)
# wien1920[pos_temp, "TP_pub.date_year"] <- 1912 # Generalstadtplan Wien 1912
# wien1920$Location.usm <- gsub("JA", "yes", wien1920$Location.usm)
# wien1920$Location.usm <- gsub("NEIN", "no", wien1920$Location.usm)
# wien1920$Location.lsm <- gsub("JA", "yes", wien1920$Location.lsm)
# wien1920$Location.lsm <- gsub("NEIN", "no", wien1920$Location.lsm)
# wien1920$CD.abam[is.na(wien1920$CD.abam)] <- ""
# wien1920$CD.bs[is.na(wien1920$CD.bs)] <- ""
# wien1920$CD.bs <- gsub(",", ";", wien1920$CD.bs)
# wien1920$Address.2019[is.na(wien1920$Address.2019)] <- ""
# wien1920$Address.2019_reloc[is.na(wien1920$Address.2019_reloc)] <- ""
# wien1920$Address_reviewed[is.na(wien1920$Address_reviewed)] <- ""
# wien1920$Address_identical[is.na(wien1920$Address_identical)] <- ""
# wien1920$Address_rejected[is.na(wien1920$Address_rejected)] <- ""
# wien1920$Address_added[is.na(wien1920$Address_added)] <- ""
# wien1920$Address.1920[is.na(wien1920$Address.1920)] <- ""
# wien1920$Address_reviewed[is.na(wien1920$Address_reviewed)] <- ""
# wien1920$Polygon.source.Scale <- paste("1:", wien1920_raw$Polygon.source.Scale, sep = "")
# 
# write.table(wien1920, file = paste(dir, "Datasets/Building stock and age 1920/BSM_1920_attribute_table_temporary.csv", sep = ""), sep = ";")

################
# datasets
################

# Import CSV file "BSM_1920_attribute_table_temporary", which is the attribute table with primary data fields
wien1920 <- read.table(paste(dir_zenodo, "Datasets/Building stock and age 1920/BSM_1920_attribute_table_temporary.csv", sep = ""), sep = ";", stringsAsFactors = F)
wien1920$Area.ha <- wien1920$Area*10^-4

# Import SHP file "City boundary 1920"
CB_1920 <- st_read(paste(dir_zenodo, "Datasets/City boundary 1920/CB_1920.shp", sep = ""), quiet = T)

# Import SHP file "Urban district boundaries 1920" 
UDB_1920 <- st_read(paste(dir_zenodo, "Datasets/Urban district boundaries 1920/UDB_1920.shp", sep = ""), quiet = T)

SABAM_1920 <- st_read(paste(dir_zenodo, "Datasets/Scope of analog building age map 1920/SABAM_1920.shp", sep = ""), quiet = T)

################
# Background data
################

# Import Online-only Table 2. Retrieved from Data Descriptor.
DD_OOT_2 <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet = "DD_OOT_2", col_names =  TRUE, range = "A3:C31")

# Import statistic on built-up area in 1912.
BUA_1912_statistic_raw <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet = "BUA_1912_statistic", col_names =  TRUE, range = "A3:K15")
BUA_1912_statistic <- BUA_1912_statistic_raw[,c("Zeitschnitt","gesamt")]
colnames(BUA_1912_statistic) <- c("year", "Area")

# Import statistic on urban district areas 1920.
UD_AREA_1920_statistic <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet = "UD_AREA_1920_statistic", col_names =  TRUE, range = "A3:B24")

# Import statistic on building counts. 
BC_statistic <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet = "BC_statistic", col_names =  TRUE, col_types = rep("numeric", times = 6), range = "A3:F24")

# Import timeseries data on building counts.
BC_TL <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet = "BC_TL", col_names = T, range = "A3:B25")
colnames(BC_TL) <- c("year", "buildings")

# Import timeseries data inhabitants.
INHAB_TL <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet =  "INHAB_TL", col_names = TRUE, range = "A3:B28" )
colnames(INHAB_TL) <- c("year", "Inhabitants")

# Import building schematic 
BS_1920s <- read_xlsx(paste(dir_github, "Background data/Tables.xlsx", sep = ""), sheet =  "BS_1920s", col_names = TRUE, range = "A1:L42862" )
...
# Transparente Farben: https://www.dataanalytics.org.uk/make-transparent-colors-in-r/ 
...
pdf(paste(dir, "Figures/Figure 01a.pdf", sep = ""), width = 600/60, height = 350/60)
par(mar=c(5,5,3,5))

plot(data[,1], data[,3], type = "p", pch = 19, xlab="", ylab = "", cex=1.5,las=1, ylim = c(0,200000)) # Häuser
par(new=T, xaxt="n")
plot(data[,1], data[,4]*100, col=2, pch = 19, ylab="", xlab="year",cex=1.5, yaxt="n", ylim = c(0,20000)) # Siedlungsfläche in ha
axis(4, pretty(0:20000), col.axis=2, las=1)

legend("topleft", c("Buildings [Number of buildings]", "Settlement Area [hectares]"), col=c(1,2), pch = 19)
...
pdf(paste(dir, "Figures/Figure 01b.pdf", sep = ""), width = 600/60, height = 350/60)
par(mar=c(5,5,3,5))

plot(data[,1], data[,3], type = "p", pch = 19, xlab="year", ylab = "", cex=1.5,las=1, ylim = c(0,120),yaxt='n') # Häuser
...
text(1701, ok + 3, "City maps", col = "black", pos = 4, font = fn)

# Kataster
rect(1817, ok+ab-hoehe, 1975, ok+ab, col = mycol, border = mycol)
rect(1975, ok+ab-hoehe, 2020, ok+ab, col = mycol2, border = mycol2)
text(1817, ok+ab + 3, "Cadaster", col = "black", pos = 4, font = fn)

# Luftbilder
rect(1938, ok+ab+ab-hoehe, 2012, ok+ab+ab, col = mycol, border = mycol) 
rect(2012, ok+ab+ab-hoehe, 2020, ok+ab+ab, col = mycol2, border = mycol2) 
text(1938, ok+ab+ab + 3, "Aerial photographs", col = "black", pos = 4, font = fn)

# Building stock maps
uk <- 60 # unterkante vom senkrechten balken
text(1829, uk+hoehe + 3, "Historic building stock maps", col="black", pos = 4, font = fn)

#1920 Stadkarte
rect(1920, uk, 1921, uk+hoehe, col = mycol2, border = mycol2) 
text(1921, uk + 3, "Building stock\nmap 1920", col = "darkblue", pos = 4)

#1920 Franzeisischer Kataster 1829 https://www.wien.gv.at/kulturportal/public/grafik.aspx?bookmark=EeNvRma8KkZmpYFEvioURBwZ9D0-b&lang=de&bmadr=
rect(1829, uk, 1830, uk+hoehe, col = mycol2, border = mycol2) 
text(1829, uk + 3, "Franzeisischer\nKataster 1829", col = "darkblue", pos = 4)

ab <- ab*1.5

# Setttlemen Area maps

text(1704, uk+ab+hoehe + 3, "Historic settlement area maps", col="black", pos = 4, font = fn)

rect(1912, uk+ab, 1913, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1912, uk+ab + 3, "1912", col = "darkblue", pos = 4)

rect(1875, uk+ab, 1876, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1875, uk+ab + 3, "1875", col = "darkblue", pos = 4)

rect(1825, uk+ab, 1826, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1825, uk+ab + 3, "1825", col = "darkblue", pos = 4)

rect(1780, uk+ab, 1781, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1780, uk+ab + 3, "1780", col = "darkblue", pos = 4)

rect(1755, uk+ab, 1756, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1755, uk+ab + 3, "1755", col = "darkblue", pos = 4)

rect(1704, uk+ab, 1705, uk+ab+hoehe, col = mycol2, border = mycol2) 
text(1704, uk+ab + 3, "1704", col = "darkblue", pos = 4)

# Hintergrundblöcke
rect(1700, 0, 2020, 50, col = mycol3, border = mycol3) 
text(1700, 50-5, "Topographic documents", col = "black", pos = 4)

rect(1700, 55, 2020, 105, col = mycol3, border = mycol3) 
text(1700, 65, "Reconstruction and mapping\nof historical settlement data", col = "black", pos = 4)
...
text(x+15, y, "analog", col = "black", pos = 1)
text(x+45, y, "digital", col = "black", pos = 1)
text(1700, 115, "Data format", col = "black", pos = 4)
...
Figure_1 <- function(){
  layout(matrix(c(1,2), 2), widths = 8, heights = c(4,5))
  par(mar=c(5,5,3,5))
...
text(1780, uk+ab+hoehe + 3, "Historic settlement Area maps", col="black", pos = 4, font = fn)
...
rect(1700, 55, 2020, 105, col = mycol3, border = mycol3) 
text(1700, 105-15, "Reconstruction\nand mapping\nof historical\nsettlement data", col = "black", pos = 4)
...
pdf(paste(dir, "Figures/Test2.pdf", sep = ""), width = 10, height = 13)
...
# Methods
## Overview
No computations for Data Descriptor.
...
### Mapping the building stock and its age in the year 1920
...
CB_1920_area <- st_area(CB_1920)
UDB_1920_area <- sum(st_area(UDB_1920))
SABAM_1920_area <- st_area(SABAM_1920)

boundary_area <- data.frame(c("CB_1920","UDB_1920","SABAM_1920"),c(CB_1920_area,UDB_1920_area,SABAM_1920_area))
colnames(boundary_area) <- c("Dataset", "Area")

```


* Total area of polygon-features by dataset `r boundary_area %>% kbl(digits = 0, format.args = list(big.mark = ',')) %>% kable_styling(bootstrap_options = "striped", full_width = F, position = "left")`

### Building stock map and its age 1920
...
wien1920[pos_1, "Polygon.source_name"] <- "Historic urban district map"
wien1920[pos_2, "Polygon.source_name"] <- "Historic cadaster"
wien1920[pos_3, "Polygon.source_name"] <- "Analog building age map 1920"
wien1920[pos_4, "Polygon.source_name"] <- "Historic fire brigade map"
...
Data field "Location.city_limit_1920"
Location inside or outside of the city limits 1920.
...
data_location.city_limits_plot <- ggplot(data_location.city_limits, aes(x=UD.2020, y=sum_area, fill = Location.city_limit_1920))+
  geom_bar(position="stack", stat="identity")+
    ggtitle("Location of building footprints - city limits 1920") +
    ylab("Area [hectares]") +
    scale_x_continuous("Urban district 2020", labels = c(1:23), breaks = c(1:23))+
    geom_text(data=totals, aes(x=UD.2020, label=total, y=total, fill=NULL), nudge_y=5, size = 3)+
   labs(fill = "Location")
...
Data field: Location.abam_1920
Location inside or outside of the building age map 1920 geographical coverage.

```{r, echo = FALSE}

data_location.amap <- wien1920 %>%
  group_by(UD.1920,Location.abam_1920) %>%
  summarise(sum_area = sum(Area.ha))
...
data_location.amap_plot <- ggplot(data_location.amap, aes(x=UD.1920, y=sum_area, fill = Location.abam_1920))+
  geom_bar(position="stack", stat="identity")+
    ggtitle("Location of building footprints - scope of building age map 1920") +
    ylab("Area [hectares]") +
    scale_x_continuous("Urban district 1920", labels = c(1:21), breaks = c(1:21))+
  geom_text(data=totals, aes(x=UD.1920, label=total, y=total, fill=NULL), nudge_y=5, size = 3)+
  labs(fill = "Location")
...
## Content-related plausibility check by external data sources
### City area 1920

```{r, echo = FALSE}
area_polygon <- as.numeric(st_area(CB_1920))*10^-4 # in ha
area_statistic <- sum(UD_AREA_1920_statistic$Gesamtgrundfläche) # in ha

```

* Difference between CB_1920 and statistical yearbook (1929): `r abs(round(area_polygon - area_statistic, digits=0))` ha (`r round(100 - (area_polygon / area_statistic *100), digits=2)`%)

### Urban district area 1920

```{r, echo = FALSE}
# Data retrieved the the SHP file "Urban district boundaries 1920"
area_polygon_district <- data.frame(UDB_1920$UDN, as.numeric(st_area(UDB_1920))*10^-4, stringsAsFactors = F) # in ha
colnames(area_polygon_district) <- c("UD.1920", "Area")

area_polygon_district <- area_polygon_district[order(area_polygon_district$UD.1920),]

# Data from statistical yearbook
area_statistic_district <- data.frame(UD_AREA_1920_statistic, stringsAsFactors = F) # in ha
colnames(area_statistic_district) <- c("UD.1920", "Area")


# combine data

data_area_poly <- data.frame(c(1:21), rep("Urban district limits map 1920", times =21), area_polygon_district$Area)
data_area_stat <- data.frame(c(1:21), rep("Statistical yearbook (1929)", times =21), area_statistic_district$Area)

col <- c("UD.1920", "Data_source", "Area")

colnames(data_area_poly) <- col
colnames(data_area_stat) <- col

data <- rbind(data_area_poly, data_area_stat)
data$Data_source <- as.character(data$Data_source)
data$Area.label <- round(data$Area, digits = 0)

plaus_dist_area_plot <- ggplot(data, aes(fill=Data_source, y=Area, x=UD.1920)) + 
    geom_bar(stat="identity", position=position_dodge())+
    ggtitle("Contrasting urban district areas 1920") +
    ylab("Area [hectares]") +
    scale_x_continuous("Urban district 1920", labels = c(1:21), breaks = c(1:21))+
    geom_text(aes(label=Area.label), hjust=-0.3, vjust= 0.25, position = position_dodge(1), size=3, angle=90)+
...
while (i <= 120) {
 ...
 wien1920_buildings_count_district <- wien1920_buildings %>%
  ...
  if (i == 80) {
    data.map <- data.frame(c(1:21), rep(paste("Building stock map (Polygons >=", i," m2)", sep =""), 21), wien1920_buildings_count_district[1:21,2]) 
    } else {
    data.map_new <- data.frame(c(1:21), rep(paste("Building stock map (Polygons >=", i," m2)", sep =""), 21), wien1920_buildings_count_district[1:21,2])
...
# Plotting figure
data.stat <- data.frame(c(1:21), rep("Statistical yearbook (1923)", 21), BC_statistic$Häuser_2) # yearbook 1914
data.stat2 <- data.frame(c(1:21), rep("Statistical yearbook (1934)", 21), BC_statistic$Häuser_3) 
...
p <- ggplot(data, aes(fill=factor(data_source, levels = data_legend), y=counts, x=UD.1920s)) + 
...
sel_pos <- c(which(BSM_BS_bc$data_source == "YoC.1920s_BS_1920s"), which(BSM_BS_bc$data_source == "CD.bs_BSM_1920"), which(BSM_BS_bc$data_source == "Statistical yearbook (1923)"))

ranking <- c("Statistical yearbook (1923)","YoC.1920s_BS_1920s","CD.bs_BSM_1920")
...
BSM_1920_bc_final_plot <- ggplot(BSM_BS_bc[sel_pos,], aes(x= UD.1920s, y=counts, fill=factor(data_source, levels = ranking)))+
          ...
          ylab("Building count")+
          scale_fill_discrete(name = "Data field,\nsource", labels = c("Buildings,\nSY_1923 dataset", "YoC.1920s,\nBS_1920s dataset", "CD.bs,\nBSM_1920 dataset"))
...
# This figure is not included in the Data Descriptor
...
# This figure is included in the Data Descriptor
Ratio_CD.bs_data_plot <- ggplot(Ratio_data[which(Ratio_data$Ratio_type == "'CD.bs': blank"), ], aes(x= reorder(UD.1920, -Ratio), y=Ratio, fill=Ratio_type))+
          ...
          ylim(c(0:1))+
          scale_fill_discrete(name = "Ratio", labels = c("YoC.1920s\nassignments"))
...
#ggsave("E:/01 Projekte/MUBS_FWF/R-Resultat/Publikationen/Building stock in Vienna in the 1920s/Figures/Figure CD.bs_fit_data_plot.png", plot =CD.bs_fit_data_plot, width = 10, height = 5
...
write.table(wien1920[,DD_OOT_2$Data_field], file = paste(dir_zenodo, "Datasets/Building stock and age 1920/BSM_1920_attribute_table_final.csv", sep = ""), sep = ";", row.names=FALSE)
...
## Plotting figures {-}
The figures are published in the corresponding Data Descriptor.
...
figure_out_1 <- ggarrange(area_district_adapt_plot, poly_source_plot, area_ud2020_editor_plot, temp_presence_district_plot, align = "v", ncol = columns, nrow = rows, labels = "auto")
ggsave(paste(dir, "Figures/Figure 07.png", sep = ""), plot = figure_out_1, width = columns * 9 , height = rows*4)
ggsave(paste(dir, "Figures/Figure 07.pdf", sep = ""), plot = figure_out_1, width = columns * 9 , height = rows*4)
...
figure_out_2 <- ggarrange(bp_plot, PoC_data_plot, data_location.city_limits_plot, data_location.amap_plot, align = "v", ncol = columns, nrow = rows, labels = "auto")
ggsave(paste(dir, "Figures/Figure 08.png", sep = ""), plot = figure_out_2, width = columns * 9 , height = rows*4)
ggsave(paste(dir, "Figures/Figure 08.pdf", sep = ""), plot = figure_out_2, width = columns * 9 , height = rows*4)

# Plot Figure 09

ggsave(paste(dir, "Figures/Figure 09.png", sep = ""), plot = plaus_dist_area_plot, width = 9 , height = 4)

ggsave(paste(dir, "Figures/Figure 09.pdf", sep = ""), plot = plaus_dist_area_plot, width = 9 , height = 4)

# Plot Figure 10
ggsave(paste(dir, "Figures/Figure 10.png", sep = ""), plot = p, width = 9 , height = 5)
ggsave(paste(dir, "Figures/Figure 10.pdf", sep = ""), plot = p, width = 9 , height = 5)

# Plot Figure 11
columns = 1
rows = 2
  
figure_loc <- ggarrange(location_confirm_plot, location_overview_analysis_plot, align = "v", ncol = columns, nrow = rows, labels = "auto")
ggsave(paste(dir, "Figures/Figure 11.png", sep = ""), plot = figure_loc, width = columns * 9 , height = rows*4)

ggsave(paste(dir, "Figures/Figure 11.pdf", sep = ""), plot = figure_loc, width = columns * 9 , height = rows*4)
...
figure_12 <- ggarrange(BSM_1920_bc_final_plot, Ratio_CD.bs_data_plot, CD.bs_fit_source_plot, CD.bs_fit_data_plot, align = "v", ncol = columns, nrow = rows, labels = "auto")

ggsave(paste(dir, "Figures/Figure 12.png", sep = ""), plot = figure_12, width = columns *9 , height = rows* 4)
ggsave(paste(dir, "Figures/Figure 12.pdf", sep = ""), plot = figure_12, width = columns * 9 , height = rows* 4)
