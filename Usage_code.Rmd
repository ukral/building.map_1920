...
wien1920_complete <- read.csv(paste(dir_zenodo, "Building stock and age 1920/BSM_1920_attribute_table.csv", sep = ""), sep = ";", stringsAsFactors = F)
...
par(new=T)
plot(data[,1], data[,4]*100, col=2, pch = 19, ylab="", xlab="year",cex=1.5,  xlim = c(1700,2050), xaxt="n", yaxt="n", ylim = c(0,20000)) # Settlement area by hectares
axis(4, pretty(0:20000), col.axis=2, las=1)

legend("topleft", c("Buildings by number", "Settlement area by hectares"), col=c(1,2), pch = 19)
...
lines(c(2020,2020), c(0,115), col="black", lwd=2, lty=2)
text(2020,0, "today", pos = 4, srt = 90)
...
text(1829, uk+hoehe + 5, "Building stock", col="black", pos = 4, font = fn)
...
text(1704, uk+ab+hoehe + 5, "Settlement areas", col="black", pos = 4, font = fn)
...
text(1700, 110, "City-wide mapping of historical information", col = "grey25", pos = 4, font = 2)
...
* Difference between CB_1920 and SY_1929: `r abs(round(area_polygon - area_statistic, digits=0))` ha (`r round(100 - (area_polygon / area_statistic *100), digits=2)`%)
...
data_area_poly <- data.frame(c(1:21), rep("UDB_1920", times =21), area_polygon_district$Area)
data_area_stat <- data.frame(c(1:21), rep("SY_1929", times =21), area_statistic_district$Area)
...
diff <- data.frame(c(1:21), data[which(data$Data_source == "UDB_1920"), "Area"], data[which(data$Data_source == "SY_1929"), "Area"], data[which(data$Data_source == "UDB_1920"), "Area"] - data[which(data$Data_source == "SY_1929"), "Area"])

colnames(diff) <- c("UD.1920", "UDB_1920", "SY_1919", "area_diff_abs")
diff$area_diff_rel <- diff$area_diff_abs / data[which(data$Data_source == "UDB_1920"), "Area"]*100

```


Absolute differences between UDB_1920 and SY_1929. `r diff %>% kbl(digits = 0, format.args = list(big.mark = ',')) %>% kable_styling(bootstrap_options = "striped", full_width = F, position = "left")`
...
ggsave(paste(dir_output, "Figure 08.png", sep = ""), plot = figure_out_2, width = columns * 9 , height = rows*5)
ggsave(paste(dir_output, "Figure 08.pdf", sep = ""), plot = figure_out_2, width = columns * 9 , height = rows*5)
